...
class NcDteDocument(models.AbstractModel):
    _name = "l10n_sv.dte.nc"
    _inherit = "l10n_sv.dte.mixin"
    _description = "Nota de Credito Electrónica"
    ...
    def generate_json(self, invoice, credentials):
        ...
        doc_rel_cod_generacion = invoice.reversed_entry_id.tgr_l10n_sv_edi_codigo_generacion
        return {
            ...
            "ventaTercero": None,
            "cuerpoDocumento": self._get_cuerpo_documento(values, doc_rel_cod_generacion),
    ...
    def _get_documento_relacionado(self, invoice, numero_control):
        return [
            {
                "tipoDocumento": invoice.reversed_entry_id.l10n_latam_document_type_id.code,
                "tipoGeneracion": int(invoice.tgr_l10n_sv_edi_tipo_generacion) or 2,
                "numeroDocumento": numero_control,
                "fechaEmision": invoice.reversed_entry_id.invoice_date.strftime("%Y-%m-%d"),
            }
        ]
